...
class ExpertDataset(object):
    ...
    def __init__(self, expert_path=None, traj_data=None, train_fraction=0.7, batch_size=64,
                 traj_limitation=-1, randomize=True, verbose=1, sequential_preprocessing=False, logger=None):
        ...
        self.episode_starts = traj_data['episode_starts']
        ...
        if traj_limitation > 0:
            ...
            for idx, episode_start in enumerate(self.episode_starts):
        ...
        self.obs_original_shape = observations.shape[-3:]
        if len(observations.shape) > 2:
            observations = np.reshape(observations, [-1, np.prod(observations.shape[1:])])
        self.acts_original_shape = actions.shape[-1]
        ...
        self.rewards = traj_data['rewards'][:traj_limit_idx]
    ...
    def log_info(self):
        ...
        if self.logger is not None:
            self.logger.log("Total trajectories: {}".format(self.num_traj))
            self.logger.log("Total transitions: {}".format(self.num_transition))
            self.logger.log("Average returns: {}".format(self.avg_ret))
            self.logger.log("Std for returns: {}".format(self.std_ret))
        ...
        try:
            for i in range(self.num_transition):
                ...
                rollout_buffer.add(
                    ...
                    # Values and probs defined to be zero, as actions considered as GT
